...
*/
function doTSummary(it) { var out='<article class="narrow"> ';if(it.link){out+='<a href="'+(it.link)+'">';}out+=' <h1>'+(it.title);if(it.parts > 1){out+=' -'+(it.parts)+' Parts';}out+='<span>, '+(it.cost)+'</span></h1> ';if(it.link){out+='</a>';}out+=' <div class="classbox content clearfix"> ';if(it.link){out+='<a href="'+(it.link)+'">';}if(it.image){out+='<img src='+(it.image)+' alt="">';}if(it.link){out+='</a>';}out+=' ';if(it.note){out+='<h3>Note: '+(it.note)+'</h3>';}out+=' <h4>Dates</h4> <p>';var arr1=it.dates;if(arr1){var value,index2=-1,l1=arr1.length-1;while(index2<l1){value=arr1[index2+=1];if(index2 > 0){out+=' OR ';}var arr2=value;if(arr2){var value,index3=-1,l2=arr2.length-1;while(index3<l2){value=arr2[index3+=1];if(index3 > 0){out+=' & ';}out+=''+(value);} } } } out+='</p> <h4>Times</h4> <p>';var arr3=it.times;if(arr3){var value,index2=-1,l3=arr3.length-1;while(index2<l3){value=arr3[index2+=1];if(index2 > 0){out+=' <strong><u>OR</u></strong> ';}out+=''+(value);} } out+='</p> ';if(it.description){out+=' <h4>Description</h4> <p>'+(it.description)+'</p> ';}out+=' ';if(it.prerequisites){out+=' <h4>Prerequisites</h4> <p>'+(it.prerequisites)+'</p> ';}out+=' ';if(it.supplies){out+=' <h4>Supplies</h4> <p>'+(it.supplies)+'</p> ';}out+=' </div></article>';return out; }
