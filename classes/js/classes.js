 ...
 *  <span>{{~it.array :value:index}}<p>{{? value.image}}<img src={{=value.image}} alt="">{{?}}<span class="title">{{=value.title}}</span> - {{~value.date :value2:index2}}{{? index2 > 0}} & {{?}}{{=value2}}{{~}}<span class="time">({{=value.time}})</span></p>{{~}}</span>
 */
